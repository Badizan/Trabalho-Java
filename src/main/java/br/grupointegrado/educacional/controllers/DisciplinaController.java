...
public class DisciplinaController {
    ...
    public ResponseEntity<Disciplina> addCurso(@PathVariable Integer id, @PathVariable Integer cursoId) {
        ...
        if(disciplina.getCurso().getId().equals(curso.getId())) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "O curso já está cadastrado na disciplina!");
        }
    ...
    public ResponseEntity<Disciplina> addProfessor(@PathVariable Integer id, @PathVariable Integer professorId) {
        ...
        Professor professor = this.professorRepository.findById(professorId)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Id do professor não encontrado!"));

        if(disciplina.getProfessor() != null && disciplina.getProfessor().getId().equals(professor.getId())) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "O professor já está cadastrado na disciplina!");
        }
    ...
    // Adicionar delete para a disciplina
    @DeleteMapping("/{id}")
